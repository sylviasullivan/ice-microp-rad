...
def meanProfile(basedir,tag,f,startindx,endindx):
    ...
    np.save('../output/H_' + tag + '_' + suffix2 + '.npy',H)
    return H


if arr[0] == True:
   H_1mom = meanProfile('/scratch/b/b380873/tropic_run2/','1mom',0,1,25)
else:
   H_1mom = np.load('../output/H_1mom' + suffix2 + '.npy')


if arr[1] == True:
   H_no2mom = meanProfile('/scratch/b/b380873/tropic_run5_no2mom/','no2mom',0,1,25)
else:
...
if arr[4] == True:
   H_Atest = meanProfile('/scratch/b/b380873/tropic_run6/','Atest',0,1,24)
else:
...
#fig.savefig('../output/lw-sw-heating-profiles2.pdf')
#plt.show()
...
#fig.savefig('../output/heating-profiles-ERA5also.pdf')
plt.show()
