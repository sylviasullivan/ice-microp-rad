...
if arr[0] == True:
   for i in np.arange(1,25):
       ...
       temp = (flx.lwflxclr.isel(time=0)).where(clch > f)
       ...
       temp = (flx.swflxclr.isel(time=0)).where(clch > f)
       temp = temp.assign_coords(plev_2=temp.plev_2.values)
       temp = temp.differentiate('plev_2')
       H_1mom[i-1,1] = g/cp*86400*temp.mean(dim={'ncells'})
   print('Saving 1mom clr-sky heating rates...')
   np.save('../output/Hclr_1mom' + suffix2 + '.npy',H_1mom)
else:
   H_1mom = np.load('../output/Hclr_1mom' + suffix2 + '.npy')
...
if arr[1] == True:
   ...
   print('Saving no2mom clr-sky heating rates...')
   np.save('../output/Hclr_no2mom' + suffix2 + '.npy',H_no2mom)
else:
   H_no2mom = np.load('../output/Hclr_no2mom' + suffix2 + '.npy')
...
if arr[2] == True:
   ...
   print('Saving novgrid clr-sky heating rates...')
   np.save('../output/Hclr_novgrid' + suffix2 + '.npy',H_novgrid)
else:
   H_novgrid = np.load('../output/Hclr_novgrid' + suffix2 + '.npy')
...
if arr[3] == True:
   ...
   print('Saving 2mom clr-sky heating rates...')
   np.save('../output/Hclr_2mom' + suffix2 + '.npy',H_2mom)
else:
   H_2mom = np.load('../output/Hclr_2mom' + suffix2 + '.npy')
...
ax[0].set_xlabel(r'LW clr-sky radiative heating [K day$^{-1}$]',fontsize=fs)
...
ax[1].set_xlabel(r'SW clr-sky radiative heating [K day$^{-1}$]',fontsize=fs)
...
fig.savefig('../output/clrsky-heating-profiles.pdf')
plt.show()
