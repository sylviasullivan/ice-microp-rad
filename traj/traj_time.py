...
dates = [datetime(2017,8,7,12,0) + timedelta(seconds=t) for t in clams_t]

ax[0].xaxis_date()
for i in np.arange(n):
    ax[0].plot(dates,icon_qi_cubic[i],color=colors1[i],alpha=0.25,lw=0.25)
    ax[0].plot(dates,clams_qi[:,i]*10**6,color=colors2[i],alpha=0.25,lw=0.25)
    # Factor of 10**6 converts to mg per kg
ax[0].plot(dates,np.nanmean(icon_qi_cubic,axis=0),color='red',lw=1.25)
ax[0].plot(dates,np.nanmean(clams_qi*10**6,axis=1),color='blue',lw=1.25)
ax[0].set_ylim([10**(-2),20])
ax[0].set_ylabel(r'Ice mass mixing ratio [mg kg$^{-1}$]',fontsize=fs)
ax[0].set_xlabel('Trajectory time',fontsize=fs)
