...
def meanProfile(basedir, acronym, f, startindx, endindx, fileprefix):
    ...
    np.save('../output/QS_' + acronym + suffix + '.npy',QS)
    return QS


for a in acronym:
   QS = meanProfile('/scratch/b/b380873/' + a + '/', a, 0, 1, 25, 'CLCONV_3D')

ll = len(others + acronym)
QS_all = np.zeros((ll, 24, 120))
for indx, o in enumerate(others + acronym):
    QS_all[indx] = np.load('../output/QS_' + o + suffix + '.npy')
...
for q, o in zip(QS_all, others + acronym):
...
fig.savefig('../output/qs-profiles_all.pdf')
plt.show()
